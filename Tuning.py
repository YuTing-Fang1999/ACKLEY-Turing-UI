...
class Tuning():
    def __init__(self, ui, setting, capture):
        ...
        # plot
        self.bset_score_plot = MplCanvas_timing(self.best_score_canvas, self.best_score_layout, color = ['r'], label = ['score'])
    ...
    def set_time_counter(self):
        ...
        while self.is_run:
            ...
            self.ui.label_time.setText(str(f"{hour}:{minute:0>2}:{sec:0>2}")) # show time_counter (by format)
